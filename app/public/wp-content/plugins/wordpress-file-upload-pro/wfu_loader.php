...
add_shortcode("wordpress_file_upload_browser", "wordpress_file_upload_browser_handler");
...
add_action('wp_ajax_wfu_ajax_action_refresh_captcha', 'wfu_ajax_action_refresh_captcha');
add_action('wp_ajax_nopriv_wfu_ajax_action_refresh_captcha', 'wfu_ajax_action_refresh_captcha');
add_action('wp_ajax_wfu_ajax_action_delete_file', 'wfu_ajax_action_delete_file');
add_action('wp_ajax_nopriv_wfu_ajax_action_delete_file', 'wfu_ajax_action_delete_file');
add_action('wp_ajax_wfu_ajax_action_get_userpermissions_page', 'wfu_ajax_action_get_userpermissions_page');
add_action('wp_ajax_wfu_ajax_action_get_filebrowser_page', 'wfu_ajax_action_get_filebrowser_page');
add_action('wp_ajax_nopriv_wfu_ajax_action_wfu_call_async', 'wfu_ajax_action_wfu_call_async');
add_action('wp_ajax_nopriv_wfu_ajax_action_load_hook_code', 'wfu_ajax_action_load_hook_code');
add_action('wp_ajax_wfu_ajax_action_get_hook_code_from_template', 'wfu_ajax_action_get_hook_code_from_template');
//browser specific ajax actions
add_action('wp_ajax_wfu_ajax_action_browser_reload', 'wfu_ajax_action_browser_reload');
add_action('wp_ajax_nopriv_wfu_ajax_action_browser_reload', 'wfu_ajax_action_browser_reload');
...
add_action('wfu_scheduled_events', 'wfu_execute_hourly_tasks');
...
//include files of extensions
wfu_include_extensions();
wfu_include_lib();
//initialize auto-updater
wfu_initialize_plugin_auto_updater();
...
function wordpress_file_upload_initialize() {
	...
	//if the request contains wfu_deactivate_hooks this means that a command is
	//given to deactivate all plugin hooks because one of them causes the
	//website to crash
	if ( isset($_GET['wfu_deactivate_hooks']) ) wfu_deactivate_all_hooks();
	//execute activated hooks
	wfu_execute_hooks();
}
 ...
 *
 * It loads all existing plugin extensions.
 *
 * @since 4.1.0
 */
function wfu_include_extensions() {
	$dir = plugin_dir_path( WPFILEUPLOAD_PLUGINFILE )."extensions/";
	if ( $handle = opendir($dir) ) {
		$blacklist = array('.', '..');
		while ( false !== ($file = readdir($handle)) )
			if ( is_file($dir.$file) && !in_array($file, $blacklist) && substr($file, 0, 1) != "_" && strtolower(substr($file, -4)) == ".php" )
				include_once $dir.$file;
		closedir($handle);
	}
	if ( $handle = opendir(plugin_dir_path( WPFILEUPLOAD_PLUGINFILE )) ) {
 ...
 *
 * It initializes the auto-updater of the plugin.
 *
 * @since 4.14.0
 */
function wfu_initialize_plugin_auto_updater() {
	$plugin_options = wfu_decode_plugin_options(get_option( "wordpress_file_upload_options" ));
...
function wordpress_file_upload_function($incomingfromhandler) {
	...
	*/
	...
	$uploadusers = explode(",", $params["uploaduser"]);
	foreach ( $uploadusers as &$uploaduser ) {
		$uploaduser = trim($uploaduser);
	}
	/** This filter is documented above */
	if ( !( ( $user->ID > 0 && ( $plugin_upload_user_role == 'administrator' || in_array('all', $uploadusers) || in_array($user->user_login, $uploadusers) ) ) || ( $user->ID == 0 && in_array('guests', $uploadusers) ) ) ) return apply_filters("_wfu_file_upload_hide_output", "");
	...
	/* Deactivate upload chunking if upload method is ftp or classic upload is used */
	if ( $params["accessmethod"] == "ftp" || $params['forceclassic'] == "true" )
		$params["chunk"] = "false";

	/* Request a new captcha challenge code if captcha is enabled */
	$captcha_challenge = "";
	$captcha_code = "";
	if ( $params["captcha"] == "true" ) {
		if ( $params["captchatype"] == "RecaptchaV1" ) {
			$captcha_code = "";
			$captcha_challenge = "RecaptchaV1";
		}
		elseif ( $params["captchatype"] == "RecaptchaV1 (no account)" ) {
			$captcha_code = preg_replace("/.*wfu_captcha_success:/", "", wfu_get_captcha_ticket($sid, "V1"));
			$captcha_challenge = "RecaptchaV1 (no account)";
		}
		elseif ( $params["captchatype"] == "RecaptchaV2" ) {
			$captcha_code = "";
			$captcha_challenge = "RecaptchaV2";
		}
		elseif ( $params["captchatype"] == "RecaptchaV2 (no account)" ) {
			$captcha_code = preg_replace("/.*wfu_captcha_success:/", "", wfu_get_captcha_ticket($sid, "V2"));
			$captcha_challenge = "RecaptchaV2 (no account)";
		}
//		/* We cannot have single button operation when captcha is enabled */
...
//			- d is the user name
		...
		$init_params["chunk"] = ( $params["chunk"] == "true" );
	}

	$additional_params['captcha_challenge'] = $captcha_challenge;
	$additional_params['captcha_code'] = $captcha_code;
...
//	$params["uploadertemplate"] = "Custom1";
	...
	foreach ( $itemplaces as $section ) {
		...
		foreach ( $items_in_section as $item_in_section ) {
			...
			if ( isset($components_indexed[$item_in_section]) && ( $components_indexed[$item_in_section]['multiplacements'] || $components_indexed[$item_in_section]['occurrencies'] == 0 ) ) {
				...
				elseif ( $item_in_section == "captcha" && $params["captcha"] == "true" ) array_push($section_array, wfu_prepare_captcha_block($params, $additional_params, $occurrence_index));
				elseif ( $item_in_section == "filelist" ) array_push($section_array, wfu_prepare_filelist_block($params, $additional_params, $occurrence_index));
	...
	if ( $params["captcha"] == "true" && strpos($params["placements"], "captcha") === false ) {
		$section_array = array( $params );
		array_push($section_array, wfu_prepare_captcha_block($params, $additional_params, 0));
		wfu_extract_css_js_from_components($section_array, $css, $js);
		$component_output .= call_user_func_array("wfu_add_div", $section_array);
	}
	...
	/* attach recaptcha reset handler if google recaptcha version 2 is activated */
	if ( $params["captcha"] == "true" && substr($params["captchatype"], -12) != "(no account)" ) {
		$handler = 'function() { window.wfu_send_captcha_reset_command_invoker_'.$sid.' = function() { wfu_send_captcha_reset_command_internal('.$sid.'); } }';
		$wfu_js .= "\n".'wfu_addLoadHandler('.$handler.');';
	}
	/* attach recaptcha reset handler and add window messaging if google recaptcha version 2 (no-account) is activated */
	elseif ( $params["captcha"] == "true" && substr($params["captchatype"], -12) == "(no account)" ) {
		$services_server = ( $plugin_options["altserver"] == "1" && trim(WFU_VAR("WFU_ALT_IPTANUS_SERVER")) != "" ? trim(WFU_VAR("WFU_ALT_IPTANUS_SERVER")) : WFU_SERVICES_SERVER_URL );
		$handler_inner = 'function(event) { wfu_process_message(event, "'.$services_server.'"); }';
		$handler = 'function() { window.wfu_send_captcha_reset_command_invoker_'.$sid.' = function() { wfu_send_captcha_reset_command_external('.$sid.', "'.$services_server.'"); }; if (!window.wfu_process_message_attached) { window.wfu_process_message_attached = true; if(window.addEventListener) { window.addEventListener("message", '.$handler_inner.', false); } else if(window.attachEvent) { window.attachEvent("onmessage", '.$handler_inner.'); } else { window["onmessage"] = '.$handler_inner.'; } } }';
		$wfu_js .= "\n".'wfu_addLoadHandler('.$handler.');';
	}
	/* add drag and drop functionality if activated */
	if ( $params["dragdrop"] == "true" && $params["forceclassic"] != "true" ) {
...
//  Check that upload_id is valid
	...
	$abort_with_success = ( $params["userdata"] == "true" && wfu_check_remove_honeypot_fields($params["userdata_fields"], 'hiddeninput_'.$sid.'_userdata_') );
		...
		//check that captcha has passed the check, if captcha is enabled
		$captcha_verify = ( isset($_POST['captchainput_'.$sid]) ? sanitize_text_field($_POST['captchainput_'.$sid]) : "" );
		$approvedcaptcha_id = "wfu_approvedcaptcha_".$unique_id;
		$wfu_checkpass = ( $wfu_checkpass && ( $params["captcha"] != "true" || ( WFU_USVAR_exists($approvedcaptcha_id) && WFU_USVAR($approvedcaptcha_id) == $captcha_verify ) ) );
		//remove any stored captcha approvals, so that they cannot be used again
		if ( WFU_USVAR_exists($approvedcaptcha_id) ) WFU_USVAR_unset($approvedcaptcha_id);
...
function wfu_post_plugin_actions($params) {
	...
	if ( $params['gallery'] == "true" ) {
		$galleryoptions = str_replace(array( '%dq%', '%brl%', '%brr%' ), array( '"', '[', ']' ), $params['galleryoptions']);
		$echo_str .= do_shortcode('[gallery '.$galleryoptions.']');
	}
