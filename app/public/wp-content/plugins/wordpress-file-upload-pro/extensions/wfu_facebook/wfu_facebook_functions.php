...
function wfu_facebook_after_upload_handler($changable_data, $additional_data, $params) {
	...
	if ( $params["messenger"] == "true" ) {
		...
		if ( count($uploaded_file_paths) > 0 || $force_notifications ) {
			...
			$replace = array ($user_login, ( $user_email == "" ? "no email" : $user_email ), $only_filename_list, $target_path_list, $blog_id, $params["pageid"], get_the_title($params["pageid"]), $uploaddetails_url, "\n", "\"", "[", "]");
