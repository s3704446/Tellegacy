...
function wfu_process_files($params, $method) {
	...
	/*
	When $method is no-ajax, one wfu_process_file function processes all upload and post-upload actions.
	When $method is ajax, then many wfu_process_file functions are executed, one for each uploaded file,
	so post-upload actions have different behaviour as follows:
		- user messages: are shown incrementally while files are uploaded
		- wpfilebase plugin update: is done once after all uploads have finished, either all files succeeded or not
		- redirection: is done once after all uploads have finished and only if all files succeeded
		- email notification: is done once after all uploads have finished, either all files succeeded or not
	*/
	...
	if ( isset($_FILES[$uploadedfile]['error']) || $only_check || $nofileupload ) {
		...
		/* convert $_FILES array to the form that it has when multiple files are selected for upload */
		if ( !is_array($_FILES[$uploadedfile]['error']) ) {
			$_FILES[$uploadedfile]['name'] = array( "0" => $_FILES[$uploadedfile]['name'] );
			$_FILES[$uploadedfile]['type'] = array( "0" => $_FILES[$uploadedfile]['type'] );
			$_FILES[$uploadedfile]['tmp_name'] = array( "0" => $_FILES[$uploadedfile]['tmp_name'] );
			$_FILES[$uploadedfile]['error'] = array( "0" => $_FILES[$uploadedfile]['error'] );
			$_FILES[$uploadedfile]['size'] = array( "0" => $_FILES[$uploadedfile]['size'] );
		}
		/* define $files_count and restrict it to 1 if multiple files upload has not been selected */
		if ( $params["multiple"] == "true" ) $files_count = count($_FILES[$uploadedfile]['tmp_name']);
		/* extract chunk data if upload chunking is enabled */
		$chunk_data = explode(",", ( isset($_POST['chunk_data']) ? $_POST['chunk_data'] : "0,0,0,0," ));
		if ( count($chunk_data) != 5 ) $chunk_data = array( "0", "0", "0", "0", "" );
		list($file_id, $file_size, $chunk_count, $chunk_id, $filename_enc) = $chunk_data;
		$file_id = wfu_sanitize_int($file_id);
		$file_size = wfu_sanitize_int($file_size);
		$chunk_id = wfu_sanitize_int($chunk_id);
		$filename_enc = wfu_sanitize_code($filename_enc);
		if ( $chunk_count > 0 ) {
			//in chunked uploads the name value is 'blob', so we replace with the real filename 
			$_FILES[$uploadedfile]['name'][0] = wfu_basename(wfu_plugin_decode_string($filename_enc));
		}
	}
	else $files_count = 0;
	...
	for ($i = 0; $i < $files_count; $i++) {
		...
		//if the file is chunked then do not perform filter actions for each separate chunk but only for the first one
		if ( $chunk_count > 0 && ( $params["chunk"] != "true" || ( WFU_USVAR_exists("chunkdata_".$unique_id."_".$file_id."_counter") && WFU_USVAR("chunkdata_".$unique_id."_".$file_id."_counter") != "" ) ) )
			$file_unique_id = '';
		...
		if ( $upload_file_size > 0 ) {
			...
			if ( $file_unique_id != '' && !$filedata_previously_defined ) {
				...
				//correct file size for chunked files
				if ( $chunk_count > 0 )
					$filesize = $file_size;
			...
			elseif ( WFU_VAR("WFU_DOS_ATTACKS_CHECK") == "true" && wfu_check_DOS_attack() ) {
				...
				/* Chunk control */
				if ( $chunk_count > 0 ) {
					if ( $params["chunk"] == "true" ) {
						if ( $unique_id != "" ) {
							$chunk_map = "chunkdata_".$unique_id."_".$file_id;
							//wfu_debug_log("pass1\n");
							if ( !WFU_USVAR_exists($chunk_map."_counter") || WFU_USVAR($chunk_map."_counter") == "" ) {
							//wfu_debug_log("pass2\n");
								// This is the first chunk, so we store in the current session data regarding the file, which are accessible by all chunks:
								//   counter: the number of successfully uploaded chunks
								//   filename: the filename
								//   originalfilename: the original filename
								//   userdata: any userdata that the user filled in
								//   abort: abort flag, for monitoring if upload errors have occurred
								//   fileuniqueid: a unique number identifying the file
								//   chunks: an array of flags of all chunks, meaning 0:pending, 1:uploaded
								//   fileparts: an array of all the partial temporary files, composing the final file
								//   lastsign: last time a chunk upload occurred
								//   completed: file is completed
								/* File size control for chunked files */
								$size_file_ok = false;
								if ( ($file_size / 1024 / 1024) <= $params["maxsize"] ) {
									if ( $params['php_env'] == '32bit' && $file_size > 2147483647 ) $size_file_phpenv_ok = false;
									else $size_file_ok = true;
								}
								WFU_USVAR_store($chunk_map."_counter", 0);
								//filename is stored here because the first chunk may change it
								WFU_USVAR_store($chunk_map."_filename", "");
								//the original filename is also stored
								WFU_USVAR_store($chunk_map."_originalfilename", $_FILES[$uploadedfile]['name'][0]);
								//userdata are stored here because the first chunk may change them
								WFU_USVAR_store($chunk_map."_userdata", "");
								//stores abort status of chunks
								WFU_USVAR_store($chunk_map."_abort", "");
								//stores file unique id
								WFU_USVAR_store($chunk_map."_fileuniqueid", $file_unique_id);
								$chunks = array();
								for ($j = 0; $j < $chunk_count; $j++) $chunks[$j] = 0;
								//stores chunk flags
								WFU_USVAR_store($chunk_map."_chunks", $chunks);
								//stores paths of file parts
								WFU_USVAR_store($chunk_map."_fileparts", array());
								//stores last chunk activity time
								WFU_USVAR_store($chunk_map."_lastsign", time());
								//flag that stores file status (0: unfinished, 1:finished)
								WFU_USVAR_store($chunk_map."_completed", 0);
							}
							//if this is the 2nd or next chunk, first check if resume data for this file exist:
							//  - if resume data exist then first check if the upload has been completed, if yes then abort the upload
							//  - if not then check if the maximum wait time since the previous chunk upload has been exceeded, if yes then abort the upload
							//  - if resume data do not exist, this means that the file needs to be aborted
							//then, get filename and userdata from stored values, in order to be the same for all chunks
							elseif ( WFU_USVAR($chunk_map."_counter") > 0 ) {
								$resume_ok = false;
								if ( WFU_USVAR($chunk_map."_completed") != 1 ) {
									$lastsign = WFU_USVAR($chunk_map."_lastsign");
									if ( WFU_VAR("WFU_UNFINISHEDFILE_INACTIVITY_PERIOD") == -1 || time() < $lastsign + WFU_VAR("WFU_UNFINISHEDFILE_INACTIVITY_PERIOD") ) $resume_ok = true;
								}
								else {
									//if completed flag is 1, this means that the server has completed the upload of the file and has performed all post-upload actions,
									//however the client side does not know that because the response to the client never reached;
									//we cannot repeat the chunk because the file parts have already been concatenated and post actions have been executed;
									//we need to inform this script not to perform any server-side changes again but prepare a successful response to the client-side
									//(this means that the chunk will not be saved again)
									$resume_ok = true;  //this flag will make sure that the chunk will not be aborted
									$ignore_server_actions = true;  //this flag will make sure that any server side actions will not be executed 
								}
								if ( $resume_ok ) {
									$only_filename = WFU_USVAR($chunk_map."_filename");
									$target_path = wfu_basedir($target_path).$only_filename;
									$userdata_fields = WFU_USVAR($chunk_map."_userdata");
									//suppress duplicatespolicy from changing the filename again
									$params["duplicatespolicy"] = "overwrite";
								}
								else {
									WFU_USVAR_store($chunk_map."_abort", "abort");
								}
							}
							//if there was an error in the previous chunk, then abort the upload
							if ( WFU_USVAR($chunk_map."_abort") == "abort" ) {
								$file_output['admin_messages'] = wfu_join_strings("<br />", $file_output['admin_messages'], WFU_ERROR_CHUNKEDUPLOAD_ABORTED);
							}
							else {
								//calculate the number of partial temporary files, it depends on PHP_INT_SIZE php variable and WFU_PARTIALFILE_MAXSIZE variable
								if ( PHP_INT_SIZE < 8 ) {
									$part_maxsize = min(2147483647, WFU_VAR("WFU_PARTIALFILE_MAXSIZE"));
									$part_chunkcount = max(floor($part_maxsize / WFU_VAR("WFU_CHUNK_SIZE")), 1);
									$part_maxsize = $part_chunkcount * WFU_VAR("WFU_CHUNK_SIZE");
									$parts_count = floor($file_size / $part_maxsize);
									$rem = $file_size - $parts_count * $part_maxsize;
									if ( $rem > 0 ) $parts_count ++;
								}
								else {
									$part_chunkcount = $chunk_count;
									$parts_count = 1;
								}
								//find number of partial file (1-indexed) and position of current chunk
								$chunk_partnum = floor($chunk_id / $part_chunkcount) + 1;
								$chunk_relid = $chunk_id - ($chunk_partnum - 1) * $part_chunkcount;
								//generate suffix of partial file from $file_unique_id and $chunk_partnum
								$parts_count_numchars = strlen("".$parts_count);
								$part_suffix = ".".WFU_USVAR($chunk_map."_fileuniqueid").sprintf("%0".$parts_count_numchars."d", $chunk_partnum);
								//check if this is the first chunk of the partial file
								$chunk_first = ($chunk_partnum - 1) * $part_chunkcount;
								$chunk_last = min($chunk_partnum * $part_chunkcount, $chunk_count) - 1;
								$is_first = true;
								//reset current chunk
								$map_chunks = WFU_USVAR($chunk_map."_chunks");
								if ( $map_chunks[$chunk_id] == 1 ) {
									WFU_USVAR_store($chunk_map."_counter", WFU_USVAR($chunk_map."_counter") - 1);
									$map_chunks[$chunk_id] = 0;
									WFU_USVAR_store($chunk_map."_chunks", $map_chunks);
								}
								for ($j = $chunk_first; $j <= $chunk_last; $j++)
									if ( $map_chunks[$j] > 0 ) {
										$is_first = false;
										break;
									}
								//notifies wfu_upload_file that it is a chunked upload, (+) sign designates that file must be created
//								$params["accessmethod"] = "chunked".( WFU_USVAR($chunk_map."_counter") == 0 ? "+" : " " ).$chunk_id;
//								wfu_debug_log('['.time().']'."part_suffix: ".$part_suffix."\n");
//								$params["accessmethod"] = "chunked".( $is_first ? "+" : " " ).$chunk_id.'|'.$part_suffix;
								$params["accessmethod"] = "chunked".( $is_first ? "+" : " " ).$chunk_relid.'|'.$part_suffix;
								$chunk_data_ok = true;
							}
						}
						else $file_output['admin_messages'] = wfu_join_strings("<br />", $file_output['admin_messages'], sprintf(WFU_ERROR_CHUNKEDUPLOAD_UNIQUEIDEMPTY, $chunk_id));
					}
					else $file_output['admin_messages'] = wfu_join_strings("<br />", $file_output['admin_messages'], WFU_ERROR_CHUNKEDUPLOAD_NOTALLOWED);
				}
				else $chunk_data_ok = true;
				if ( !$chunk_data_ok ) {
					//abort the file and perform no resume of chunks
					$file_output['message_type'] = "errorabort";
...
//		if ( $upload_path_ok and $allowed_file_ok and $size_file_ok and ( $chunk_data_ok ) ) {
		if ( substr($file_output['message_type'], 0, 5) != "error" ) {
			...
			if ( is_uploaded_file($fileprops['tmp_name']) || $only_check || $nofileupload ) {
				...
				if ( $file_copied ) {
					...
					if ( $params["notify"] == "true" && (!$only_check || $nofileupload) && ( $chunk_count == 0 || WFU_USVAR($chunk_map."_counter") == $chunk_count - 1 ) ) {
...
//				unlink($source_path);			
		...
		//post actions for chunked upload
		if ( $chunk_count > 0 ) {
			if ( $file_output['message_type'] == "success" || $file_output['message_type'] == "warning" )
				WFU_USVAR_store($chunk_map."_counter", WFU_USVAR($chunk_map."_counter") + 1);
			$params_output_array["general"]['files_count'] = ( WFU_USVAR($chunk_map."_counter") == $chunk_count ? 1 : 0 );
			//get session id
			//if this is the first chunk store filename and userdata in session variables and also unfinished file data in a global position
			if ( WFU_USVAR($chunk_map."_counter") == 1 ) {
				WFU_USVAR_store($chunk_map."_filename", $only_filename);
				WFU_USVAR_store($chunk_map."_userdata", $userdata_fields);
				if ( $file_output['message_type'] == "success" || $file_output['message_type'] == "warning" ) {
					$map_chunks = WFU_USVAR($chunk_map."_chunks");
					$map_chunks[$chunk_id] = 1;
					WFU_USVAR_store($chunk_map."_chunks", $map_chunks);
//					array_push(WFU_USVAR($chunk_map."_fileparts"), $target_path.$part_suffix);
					$map_fileparts = WFU_USVAR($chunk_map."_fileparts");
					$map_fileparts[$chunk_partnum - 1] = $target_path.$part_suffix;
					WFU_USVAR_store($chunk_map."_fileparts", $map_fileparts);
				}
				WFU_USVAR_store($chunk_map."_lastsign", time());
				// In order to enable resuming of large files and also keep control and erase unfinished files, additional data need to be stored globally.
				// Resuming can happen when the following conditions are satisfied:
				//  1a. the session id remains the same (for strict resuming)
				//  1b. the user id remains the same (for loose resuming, only valid for registered users, not guests)
				//   2. resuming is enabled from the plugin's shortcode
				//   3. the maximum time limit for keeping unfinished files has not been exceeded
				//   4. files have the same filename
				//   5. files have the same size
				// The unfinished file data are stored as option array. Every key is a session id. Every item contains the list of unfinished files of the session.
				// Every unfinished file contains the following data:
				//   - userid: the id of the user
				//   - uniqueid: the id of the upload
				//   - filename: the original filename, we do the comparison based on this
				//   - finalfilename: the final filename, after any changes by the plugin or a filter
				//   - filesize: the total file size
				//   - parts: an array of the filepaths of all the file parts. In 64-bit server versions, it will be one part.
				//            In 32-bit server versions, it will be more parts for files larger than 2Gb
				//   - chunks: holds an array with the status of each chunk, 0 means pending, 1 means uploadedfile
				//   - lastsign: holds the last time a chunk was uploadedfile
				//   - userdata: holds userdata
				//   - fileuniqueid: the unique id assigned to the file
				//   - completed: complete flag (0: pending, 1: complete)
				//   - data: additional data
				if ( $sesid != "" ) {
					$unfinishedfiledata['userid'] = $user_id;
					$unfinishedfiledata['uniqueid'] = $unique_id;
					$unfinishedfiledata['filename'] = $_FILES[$uploadedfile]['name'][0];
					$unfinishedfiledata['finalfilename'] = $only_filename;
					$unfinishedfiledata['filesize'] = $file_size;
					$unfinishedfiledata['parts'] = WFU_USVAR($chunk_map."_fileparts");
					$chunks = array();
					for ($j = 0; $j < $chunk_count; $j++) $chunks[$j] = 0;
					if ( $file_output['message_type'] == "success" || $file_output['message_type'] == "warning" ) $chunks[$chunk_id] = 1;
					$unfinishedfiledata['chunks'] = $chunks;
					$unfinishedfiledata['lastsign'] = WFU_USVAR($chunk_map."_lastsign");
					$unfinishedfiledata['userdata'] = $userdata_fields;
					$unfinishedfiledata['fileuniqueid'] = WFU_USVAR($chunk_map."_fileuniqueid");
					$unfinishedfiledata['completed'] = 0;
					$unfinishedfiledata['data'] = '';
					wfu_store_unfinishedfiledata($sesid, $unfinishedfiledata, true);
				}
			}
			elseif ( $file_output['message_type'] == "success" || $file_output['message_type'] == "warning" ) {
				$map_chunks = WFU_USVAR($chunk_map."_chunks");
				$map_chunks[$chunk_id] = 1;
				WFU_USVAR_store($chunk_map."_chunks", $map_chunks);
				$target_path_exists = false;
				foreach ( WFU_USVAR($chunk_map."_fileparts") as $target_path_item )
					if ( $target_path_item == $target_path.$part_suffix ) {
				...
				WFU_USVAR_store($chunk_map."_lastsign", time());
				if ( $sesid != "" ) {
					$unfinishedfiledata = wfu_get_unfinishedfiledata($sesid, WFU_USVAR($chunk_map."_fileuniqueid"), true);
					if ( $unfinishedfiledata != "" ) {
						$unfinishedfiledata['chunks'][$chunk_id] = 1;
						$unfinishedfiledata['parts'] = WFU_USVAR($chunk_map."_fileparts");
						$unfinishedfiledata['lastsign'] = WFU_USVAR($chunk_map."_lastsign");
						wfu_store_unfinishedfiledata($sesid, $unfinishedfiledata, true);
					}
			...
			//inform next chunks to abort if this upload failed
			if ( substr($file_output['message_type'], 0, 10) == "errorabort" )
				WFU_USVAR_store($chunk_map."_abort", "abort");
			//set success status of the whole chunked file, after it has completely uploaded
			$file_finished_successfully = ( $params_output_array["general"]['files_count'] == 1 && WFU_USVAR($chunk_map."_abort") == "" );
			//recalculate non-success status of the file, for chunked uploads file finishes unsuccessfully only when message_type is errorabort
			$file_finished_unsuccessfully = ( substr($file_output['message_type'], 0, 10) == "errorabort" );
			//restore file unique id for using it in post-upload actions for identifying the file
			$file_unique_id = WFU_USVAR($chunk_map."_fileuniqueid");
			//if file has been uploaded successfully then concatenate all temporary files to the final one and delete them
			if ( $file_finished_successfully && !$ignore_server_actions ) {
				if ( !wfu_concatenate_temp_files(WFU_USVAR($chunk_map."_fileparts"), $target_path, $params["ftpinfo"]) ) {
					$file_finished_successfully = false;
					$file_finished_unsuccessfully = true;
					$file_output['message_type'] = "errorabort";
					WFU_USVAR_store($chunk_map."_abort", "abort");
					/* prepare and prepend details of failed file upload */
					$file_output['message'] = wfu_join_strings("<br />", $file_output['message'], WFU_ERROR_CHUNKEDUPLOAD_CONCATFAILED);
				}
				else {
					WFU_USVAR_store($chunk_map."_completed", 1);
					if ( $sesid != "" ) {
						//store completed flag in unfinishedfiledata structure
						$unfinishedfiledata = wfu_get_unfinishedfiledata($sesid, WFU_USVAR($chunk_map."_fileuniqueid"), true);
						if ( $unfinishedfiledata != "" ) {
							$unfinishedfiledata['completed'] = 1;
							wfu_store_unfinishedfiledata($sesid, $unfinishedfiledata, true);
						}
					}
				}
		...
		if ( $file_finished_successfully && !$ignore_server_actions && !$nofileupload ) {
			...
			/* Here an additional check is performed for determining whether the
			   uploaded file is a valid image using Selective's Image Type
			   library. This check is only performed on image types supported by
			   the specific library. */
			if ( WFU_VAR("WFU_DISABLE_IMAGETYPELIB_CHECK") != "true" ) {
				include_once ABSWPFILEUPLOAD_DIR.'extensions/wfu_imagetype/wfu_imagetype.php';
				if ( wfu_file_extension_imagetypelib($only_filename) && !wfu_filecheck_imagetypelib($target_path) ) {
					$file_finished_successfully = false;
					$file_finished_unsuccessfully = true;
					wfu_unlink($target_path, "wfu_process_files:2");
					$file_output['message_type'] = "errorabort";
					$file_output['message'] = wfu_join_strings("<br />", $file_output['message'], WFU_ERROR_FILE_REJECT);
					$file_output['admin_messages'] = wfu_join_strings("<br />", $file_output['admin_messages'], WFU_ERROR_ADMIN_FILE_NOIMAGE);
				}
			}
		...
		/* prepare uploaded file properties to be used for front-end browser */
		if ( $file_finished_successfully && !$ignore_server_actions && !$nofileupload && !$store_nothing ) {
			$props['inc'] = "";
			$props['file'] = $only_filename;
			$props['file_code'] = wfu_safe_store_filepath(wfu_path_abs2rel($target_path));
			$props['file_sort'] = $only_filename;
			$stat = wfu_stat($target_path, "wfu_process_files");
			$props['size'] = wfu_human_filesize($stat['size']);
			$props['size_sort'] = $stat['size'];
			$props['date'] = date(get_option('date_format', 'd/m/Y'), $stat['mtime']);
			$props['date_sort'] = $stat['mtime'];
			$props['user'] = ( $consent_revoked ? "0" : ( $user_id == 0 ? "0" : $user_login ) );
			$props['user_sort'] = $props['user'];
			$posttitle = "";
			$post_obj = get_post($params['pageid']);
			if ( $post_obj ) $posttitle = $post_obj->post_title;
			$props['post'] = $posttitle;
			$props['post_sort'] = $posttitle;
			$fields_list = array();
			foreach ( $userdata_fields as $ind => $userdata_field ) {
				$fields_obj = new stdClass();
				$fields_obj->property = $userdata_field["label"];
				$fields_obj->propvalue = ( $consent_revoked ? "" : $userdata_field["value"] );
				array_push($fields_list, $fields_obj);
				$fieldid = (int)$ind + 1;
				$props['custom'.$fieldid] = ( $consent_revoked ? "" : $userdata_field["value"] );
			}
			$props['fields'] = wfu_plugin_encode_string(wfu_prepare_userdata_list($fields_list));
			$file_output['uploaded_file_props'] = wfu_encode_array_to_string($props);
		}

		$params_output_array[$i] = $file_output;
...
//			do_action('wfu_after_file_upload', $file_unique_id, $file_output['message_type'], $file_output['message'], $file_output['admin_messages']);
		...
		/* perform additional actions on successful file upload */
		if ( $fileid > -1 && $file_finished_successfully && !$ignore_server_actions && !$nofileupload ) {
			/* attempt to create file thumbnails if this option is active */
			if ( $plugin_options["createthumbnails"] == "1" ) wfu_update_file_thumbnails($fileid);
			/* transfer file to active service accounts of extensions */
			do_action("_wfu_schedule_file_transfer", $fileid, $target_path, $userdata_fields, $params);
		}
	...
	/*
	States of file upload process while it is executed (in case of ajax method):
		State0: upload in progress with no messages: wait status - wait colors
		State1: upload in progress with messages: warning status - warning colors
		State2: upload in progress with some files not uploaded: caution status - warning colors
		State3: upload in progress with no files uploaded so far: caution status - warning colors
	States of file upload process while it is executed (in case of no-ajax method):
		State0: upload in progress (before page reloads): wait status - wait colors
	States of file upload process when it has been completed:
		State4: all files uploaded: success status - success colors
		State5: all files uploaded with messages: warning status - warning colors
		State6: some files not uploaded: caution status - warning colors
		State7: no files uploaded: error status - error colors
		State8: there are no files to upload: error status - error colors
		State9: test state: test status - test colors
		State10: JSON parse error: error status - error colors
		State11: redirecting: wait status - wait colors
		State12: upload failed: error status - error colors
	States of process when this is a no file upload (only data submit):
		State13: sending data: wait status - wait colors
		State14: data submit succeeded: success status - success colors
		State15: data submit failed: error status - error colors
	States of file upload process after upload (in case of no-ajax method):
		State16: cancelling upload: wait status - wait colors
		State17: upload cancelled: error status - error colors
		State18: upload succeeded: success status - success colors (this state
		    occurs when upload is aborted but a success message needs to be
			returned)
		State19: upload completed but no files were saved due to personal data
		    policy: success status - warning colors

	In case of ajax, when there are multiple upload actions (one for each file), the possible states of the whole process depending on current file result are the following:
		FileResult A (success): State0, State1, State2, State4, State5, State6
		FileResult B (warning): State1, State2, State5, State6
		FileResult C (error):   State2, State3, State6, State7
	*/
